...
import { formatTime, semverGte } from "./utils/Utils";
import { FlightLog } from "./FlightLog";

export class FlightLogFieldPresenter {
  static presentFlags(flags, flagNames) {
  ...
  static presentChangeEvent(flags, lastFlags, flagNames) {
    let eventState = "";
    let found = false;
  ...
  static presentEnum(value, enumNames) {
    if (enumNames[value] === undefined) return value;

    return enumNames[value];
  }

  static decodeField(flightLog: FlightLog, fieldName, value) {
    if (value == null) {
      return value;
    }
    ...
    switch (fieldName) {
      ...
      case "gyroADC[1]":
      case "gyroADC[2]":
        return Math.round(flightLog.gyroRawToDegreesPerSecond(value));
      ...
      case "debug[3]":
        ...
        break;
      default:
        throw `Unrecognized field when decode: ${fieldName}`;
    }
  }

  /**
   * Attempt to decode the given raw logged value into something more human
   * readable, or return an empty string if no better representation is
   * available.
   *
   * @param fieldName Name of the field
   * @param value Value of the field
   */
  static decodeFieldToFriendly(
    flightLog: FlightLog,
    fieldName,
    value,
    currentFlightMode
    ...
    switch (fieldName) {
      ...
      // TODO(j): Didn't see these here:
      // https://github.com/betaflight/betaflight/blob/master/src/main/blackbox/blackbox.c#L224
      ...
      case "vbatLatest":
        if (
          ...
          semverGte(flightLog.getSysConfig().firmwareVersion, "4.0.0")
        ) {
          return (
            (value / 100).toFixed(2) +
            "V" +
            ", " +
            (value / 100 / flightLog.getNumCellsEstimate()).toFixed(2) +
            "V/cell"
          );
        } else if (
          (flightLog.getSysConfig().firmwareType == FIRMWARE_TYPE_BETAFLIGHT &&
            semverGte(flightLog.getSysConfig().firmwareVersion, "3.1.0")) ||
          (flightLog.getSysConfig().firmwareType == FIRMWARE_TYPE_CLEANFLIGHT &&
            semverGte(flightLog.getSysConfig().firmwareVersion, "2.0.0"))
        ) {
          return (
            (value / 10).toFixed(2) +
            "V" +
            ", " +
            (value / 10 / flightLog.getNumCellsEstimate()).toFixed(2) +
            "V/cell"
          );
        } else {
          return (
            (flightLog.vbatADCToMillivolts(value) / 1000).toFixed(2) +
            "V" +
            ", " +
            (
      ...
      case "amperageLatest":
        if (
          (flightLog.getSysConfig().firmwareType == FIRMWARE_TYPE_BETAFLIGHT &&
            semverGte(flightLog.getSysConfig().firmwareVersion, "3.1.7")) ||
          (flightLog.getSysConfig().firmwareType == FIRMWARE_TYPE_CLEANFLIGHT &&
            semverGte(flightLog.getSysConfig().firmwareVersion, "2.0.0"))
        ) {
          return (
            ...
            "A" +
            ", " +
            (value / 100 / flightLog.getNumMotors()).toFixed(2) +
            "A/motor"
          );
        } else if (
          flightLog.getSysConfig().firmwareType == FIRMWARE_TYPE_BETAFLIGHT &&
          ...
          return (
            (flightLog.amperageADCToMillivolts(value) / 1000).toFixed(2) +
            "A" +
            ", " +
            (
  ...
  static decodeDebugFieldToFriendly(
    flightLog,
    fieldName,
    value,
    currentFlightMode
    ...
    if (flightLog) {
      let debugModeName = DEBUG_MODE[flightLog.getSysConfig().debug_mode]; // convert to recognisable name
      switch (debugModeName) {
        case "NONE":
        case "AIRMODE":
        case "VELOCITY":
        ...
        case "DUAL_GYRO_RAW":
          return (
            Math.round(flightLog.gyroRawToDegreesPerSecond(value)) + "deg/s"
          );
        case "ACCELEROMETER":
          return flightLog.accRawToGs(value).toFixed(2) + "g";
        case "MIXER":
  ...
  static fieldNameToFriendly(fieldName, debugMode) {
    if (debugMode) {
